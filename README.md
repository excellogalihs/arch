...
Optional: split your config into multiple files as it grows, using Lua's `require()`:

```lua
-- refers to ~/.config/hypr/modules/keybinds.lua
require("modules.keybinds")
```

Each `require()`'d file runs in its own scope, so a typo in one won't crash the rest of your config.
